...
# def subProblem(T, iterWt, itemIDX, itemWt, itemVal, cntr_check):
...
def knapsack(items, maxWt):
    ...
    print("what is item inter ", itemIter)
    # weight iterator
    weightIter = buildWeightIter(maxWt)
    cntr_outter = 0
    cntr_inner = 0
    print("This is itemIter, outter loop var: ", itemIter)
    print("This is weightIter, inner var: ", weightIter)
    # print("This is the table: \n", table)
    # print("hb this: subProblem(table, 1, 1, 20, 12) \n", subProblem(table, 1, 1, 20, 12))
    # table[1][1] = subProblem(table, 1, 1, 20, 12)
    # print("hb this: table[1][1] \n", table[1][1])
    for itmIdx in itemIter:
        # query item values from list
        # print("This is itmIdx, loop iter var: ", itmIdx)
        # print("This is itemIter, loop var: ", itemIter)
        # print("what will cause the 'keyError: 0', itmIdx: ", itmIdx)
        item, itemWt, itemVal = items[itmIdx]
        for w in weightIter:
            # expand table values by solving subproblem
            # table[itmIdx][w] = subProblem(table, w, itmIdx, itemWt, itemVal, cntr_outter)
            table[itmIdx][w] = subProblem(table, w, itmIdx, itemWt, itemVal)
            cntr_inner += 1
            # print("This is itmIdx, loop iter var: ", itmIdx)
            # print("This is w, loop iter var w var: ", w)
            # print("This is the table at T[itmIdx][w]: \n", table[1][1])
            # print("how many times through inner: ", cntr_inner)
        # cntr_inner = 0
        # print("This is the table after the inner loop: \n", table)
        # print("This is the table at T[1][1] after the inner loop: \n", table[1][1])
        cntr_outter += 1
        # print("how many times through outter: ", cntr_outter)
        # print("what is the last w: ", w)
        if cntr_outter > 20:
            print("what does the end of the table look like \n", table[-1][-1])
...
def main():
    ...
    parser.add_argument('-i', '--items', help='File holding list of possible Items (name, wt, value)',
                        ...
                        # test case 1
                        # default='defaultItems_test_1_B = 5.txt', dest='itemsListFileName')
                        # test case 2
                        #default='defaultItems_test_2_B = 200.txt', dest='itemsListFileName')
                        # test case 3
                        default='defaultItems_test_3_B = 0.txt', dest='itemsListFileName')
#    parser.add_argument('-w', '--weight', help='Maximum (integer) weight of items allowed', type=int, default=400,
#                        dest='maxWeight')
                        # test case 1
#    parser.add_argument('-w', '--weight', help='Maximum (integer) weight of items allowed', type=int, default=5,
#                     dest='maxWeight')
                        # test case 2
#    parser.add_argument('-w', '--weight', help='Maximum (integer) weight of items allowed', type=int, default=200,
#                       dest='maxWeight')
                        # test case 3
    parser.add_argument('-w', '--weight', help='Maximum (integer) weight of items allowed', type=int, default=0,
                        dest='maxWeight')
